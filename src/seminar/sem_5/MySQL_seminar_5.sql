...
CREATE TABLE `seminar_5`.`employees` (
...
INSERT INTO `seminar_5`.`employees` (`EmployeeID`, `LastName`, `FirstName`, `BirthDate`, `Photo`, `Notes`) 
VALUES
...
CREATE TABLE `seminar_5`.`orderdetails` (
...
INSERT INTO `seminar_5`.`orderdetails` (`OrderDetailID`,`OrderID`,`ProductID`,`Quantity`)
VALUES
(1,10248,11,12),
(2,10248,42,10),
(3,10248,72,5),
...
INSERT INTO `seminar_5`.`orders` (`OrderID`,`CustomerID`,`EmployeeID`,`OrderDate`,`ShipperID`)
VALUES
...
CREATE TABLE `seminar_5`.`products` (
...
INSERT INTO `seminar_5`.`products` (`ProductID`,`ProductName`,`SupplierID`,`CategoryID`,`Unit`,`Price`)
VALUES
...
CREATE TABLE `seminar_5`.`orderStatus` (
...
/*  Задание 2
    Вставьте образец данных в поле «OrderStatus». Поле для OrderID 101 со статусом 'Shipped'.
*/

INSERT INTO OrderStatus (OrderStatusID, 
OrderID, Status)
VALUES (NULL, 101, 'Shipped');
...
/*  Задание 4
    Создайте представление с именем «DeliveredOrders». которое отображает OrderID 
...
INSERT INTO `seminar_5`.`orderstatus` (`OrderStatusID`, `OrderID`, `Status`) 
VALUES ('2', '10251', 'Delivered');

-- создание view
CREATE VIEW DeliveredOrders AS
    SELECT o.OrderID, o.OrderDate, Status
    FROM Orders AS o
...
/*  Задание 5
    Создайте процедуру с именем «UpdateOrderStatus», который принимает OrderID и Status в качестве параметров 
    и обновляет статус в 'OrderStatus'.
*/        

CREATE PROCEDURE UpdateOrderStatus (pOrderID INT, pStatus VARCHAR(50))
UPDATE OrderStatus
SET Status = pStatus
WHERE OrderID = pOrderID;

call seminar_5.UpdateOrderStatus(10250, 'In stock');
...
/*  Задание 6
    Создайте таблицу с именем «EmployeeRoles». со столбцами EmployeeRoleID, EmployeeID (INT), Role (VARCHAR).
*/    

DROP TABLE IF EXISTS Employee_Roles;
CREATE TABLE Employee_Roles (
    EmployeeRole_ID INT AUTO_INCREMENT PRIMARY KEY,
    Employee_ID INT,
    Role VARCHAR(50)
);
...
/*  Задание 8
    Создайте представление с именем «EmployeeRolesView». который отображает 
    идентификатор сотрудника, фамилию и роль для сотрудников с должностью.
*/ 

CREATE VIEW EmployeeRolesView AS
    SELECT 
        e.EmployeeID, e.LastName, er.Role
    FROM
        Employees AS e
            JOIN
        Employee_Roles AS er ON e.EmployeeID = er.Employee_ID;
  
SELECT *
FROM EmployeeRolesView;
 
 
/*  Задание 9
    Создайте процедуру с именем «AssignEmployeeRole», который принимает идентификатор сотрудника и роль в качестве
    параметров и вставляет новую должность в 'EmployeeRoles'.
*/ 

CREATE PROCEDURE AssignEmployeeRole (pEmployee_ID INT, pRole VARCHAR(50))
INSERT INTO Employee_Roles 
(Employee_ID, Role)
VALUES (pEmployee_ID, pRole);

call seminar_5.AssignEmployeeRole(1, 'Analyst');
...
/*  Задание 10
    Создайте представление с именем «HighValueOrdersView». который отображает 
    OrderID, CustomerID и OrderDate для заказов, общая стоимость которых превышает 500 долларов США.
*/ 
