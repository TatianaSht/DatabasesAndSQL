...
CREATE TABLE `seminar_3`.`customers` (
...
INSERT INTO `seminar_3`.`customers` (`CustomerID`,`CustomerName`,`ContactName`,`Address`,`City`,
`PostalCode`,`Country`)
VALUES
...
CREATE TABLE `seminar_3`.`employees` (
...
INSERT INTO `seminar_3`.`employees` (`EmployeeID`, `LastName`, `FirstName`, `BirthDate`, `Photo`, `Notes`) 
VALUES
...
CREATE TABLE `seminar_3`.`orderdetails` (
...
INSERT INTO `seminar_3`.`orderdetails` (`OrderDetailID`,`OrderID`,`ProductID`,`Quantity`)
VALUES
(1,10248,11,12),
(2,10248,42,10),
(3,10248,72,5),
...
INSERT INTO `seminar_3`.`orders` (`OrderID`,`CustomerID`,`EmployeeID`,`OrderDate`,`ShipperID`)
VALUES
...
CREATE TABLE `seminar_3`.`products` (
...
INSERT INTO `seminar_3`.`products` (`ProductID`,`ProductName`,`SupplierID`,`CategoryID`,`Unit`,`Price`)
VALUES
...
CREATE TABLE `seminar_3`.`suppliers` (
...
INSERT INTO `seminar_3`.`suppliers` (`SupplierID`,`SupplierName`,`ContactName`,`Address`,`City`,`PostalCode`,
`Country`,`Phone`) 
VALUES 
 ...
 ('9','PB KnÃƒÆ’Ã‚Â¤ckebrÃƒÆ’Ã‚Â¶d AB','Lars Peterson','Kaloadagatan 13','GÃƒÆ’Ã‚Â¶teborg','S-345 67','Sweden',
 '031-987 65 43'), 
 ('10','Refrescos Americanas LTDA','Carlos Diaz','Av. das Americanas 12.890','SÃƒÆ’Ã‚Â£o Paulo','5442','Brazil',
 '(11) 555 4640'), 
 ('11','Heli SÃƒÆ’Ã‚Â¼ÃƒÆ’Ã…Â¸waren GmbH & Co. KG','Petra Winkler','TiergartenstraÃƒÆ’Ã…Â¸e 5','Berlin','10785',
 'Germany','(010) 9984510'), 
 ('12','Plutzer LebensmittelgroÃƒÆ’Ã…Â¸mÃƒÆ’Ã‚Â¤rkte AG','Martin Bein','Bogenallee 51','Frankfurt','60439',
 'Germany','(069) 992755'), 
 ...
 ('17','Svensk SjÃƒÆ’Ã‚Â¶fÃƒÆ’Ã‚Â¶da AB','Michael BjÃƒÆ’Ã‚Â¶rn','BrovallavÃƒÆ’Ã‚Â¤gen 231','Stockholm',
 'S-123 45','Sweden','08-123 45 67'), 
 ('18','Aux joyeux ecclÃƒÆ’Ã‚Â©siastiques','GuylÃƒÆ’Ã‚Â¨ne Nodier','203, Rue des Francs-Bourgeois','Paris',
 ...
 ('29','ForÃƒÆ’Ã‚Âªts d''ÃƒÆ’Ã‚Â©rables','Chantal Goulet','148 rue Chasseur','Ste-Hyacinthe','J2S 7S8','Canada',
 '(514) 555-2955');
...
/*
	Задание 1
	Приджойним к данным о заказах данные о покупателях. Данные, которые нас интересуют —
	имя заказчика и страна, из которой совершается покупка.
    (Таким образом у нас в Orders появились данные о CustomerID, их изначально не было. Помним что 
    если Джойн не указывается, то это по умолчанию Иннер Джойн.)
*/

-- Показ только заказчика и страны
SELECT 
...
-- 1 вариант - добавление к данным Orders заказчика и страны из Customers    
SELECT 
    o.OrderID,
    o.CustomerID,
    o.EmployeeID,
...
-- 2 вариант - добавление к данным Orders заказчика и страны из Customers  
SELECT 
    o.*, c.CustomerName, c.Country
FROM
    Orders AS o
        JOIN
    Customers AS c ON o.CustomerID = c.CustomerID;
...
/*
	...
	И выведем сверху в результирующей таблице название лидирующей.
*/

-- 1 вариант
SELECT 
    COUNT(*) AS cnt_orders, Country
FROM
    Customers AS c
        JOIN
    Orders AS o ON c.CustomerID = o.CustomerID
...
-- 2 вариант
SELECT 
    c.Country, COUNT(o.OrderID) AS NumberOfOrders
FROM
    Orders AS o
        JOIN
    Customers AS c ON o.CustomerID = c.CustomerID
...
/*
	Задание 3
	А теперь напишем запрос, который обеспечит целостное представление деталей заказа, 
...
/*
	Задание 4
	Наша следующая задача — проанализировать данные заказа, рассчитать ключевые показатели, связанные с выручкой, 
    и соотнести результаты с ценовой информацией из таблицы Products.
...
SELECT 
    ...
    round(MAX(p.Price * od.Quantity))  AS max_receipt,
    round(MIN(p.Price * od.Quantity))  AS min_receipt
FROM
    Customers AS c
        JOIN
    Orders AS o ON c.CustomerID = o.CustomerID
...
/*
	Задание для демонстрации Union
	Вывод покупателей страны Бразилия и покупателей страны Мексика (в данных запросах однаковые поля)
*/
...
/*
	Задание 6
	Напишем SQL-запрос для создания отчета об исследовании продукта, показывающего потенциальный интерес 
    к каждому продукту в разных странах. Используем CROSS JOIN операцию для создания комбинаций стран и продуктов. 
...
/*
	Задание 8
	Следующим запросом давайте создадим набор результатов, который включает уникальные записи о клиентах как для США, 
    так и для Канады. В данном случае оператор UNION объединяет результаты двух отдельных запросов, представляя 
    единый список клиентов из обеих стран, удаляя при этом любые дубликаты.
*/
