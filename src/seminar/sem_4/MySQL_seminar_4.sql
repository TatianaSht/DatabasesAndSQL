...
CREATE TABLE `seminar_4`.`customers` (
...
INSERT INTO `seminar_4`.`customers` (`CustomerID`,`CustomerName`,`ContactName`,`Address`,`City`,
`PostalCode`,`Country`)
VALUES
...
CREATE TABLE `seminar_4`.`orderdetails` (
...
INSERT INTO `seminar_4`.`orderdetails` (`OrderDetailID`,`OrderID`,`ProductID`,`Quantity`)
VALUES
(1,10248,11,12),
(2,10248,42,10),
(3,10248,72,5),
...
INSERT INTO `seminar_4`.`orders` (`OrderID`,`CustomerID`,`EmployeeID`,`OrderDate`,`ShipperID`)
VALUES
...
CREATE TABLE `seminar_4`.`products` (
...
INSERT INTO `seminar_4`.`products` (`ProductID`,`ProductName`,`SupplierID`,`CategoryID`,`Unit`,`Price`)
VALUES
...
-- вариант с оконной функцией и distinct
SELECT DISTINCT
...
-- вариант с группировкой
SELECT 
    MIN(OrderDate), MAX(OrderDate), COUNT(*), CustomerID
FROM
    orders
GROUP BY CustomerID;
...
SELECT DISTINCT
    c.CustomerID, City, COUNT(*) OVER (PARTITION BY c.CustomerID) as total_orders
FROM
    orders AS o
        JOIN
...
SELECT 
	EmployeeID,
RANK() OVER (ORDER BY COUNT(OrderID) DESC) AS EmployeeRank
FROM Orders
GROUP BY EmployeeID;
...
/*
	Задание 5
	Определите среднюю цену товаров внутри каждой категории, рассматривая только категории, 
    в которых более трех товаров.
    -- применение CTE (общее табличное выражение) именованный вложенный запрос
*/

WITH product_price as (
	SELECT DISTINCT
        CategoryID,
		ROUND(AVG(Price) OVER(PARTITION BY CategoryID), 2) as avg_price ,
...
SELECT 
    ProductName, 
    CategoryID, 
    round(SUM(Price * Quantity) / SUM(SUM(Price * Quantity)) OVER (PARTITION BY CategoryID) * 100, 2) as cash_pwrc 
FROM 
    orderdetails as od 
...
ORDER BY 
    CategoryID, 
    SUM(Price * Quantity) / SUM(SUM(Price * Quantity)) OVER (PARTITION BY CategoryID) DESC;


/*
	Задание 7
	Для каждого заказа сделайте новую колонку в которой определите общий объем продаж
...
MONTH(o.OrderDate) AS Year,
    round(SUM(od.Quantity * p.Price) OVER (PARTITION BY MONTH(o.OrderDate), 
    YEAR(o.OrderDate) ORDER BY YEAR(o.OrderDate)), 2) AS TotalSales
FROM Orders o
...
/*
	Задание 8
	Вам поручено анализировать совокупные продажи в двух европейских городах (Лондоне и Мадриде) 
    к концу каждой недели с начала апреля 2023 года. 
    Используйте оконные функции SQL для расчета и отслеживания совокупных продаж с течением  
...
FROM (
    ...
    WHERE (c.City = 'London' OR c.City = 'Madrid') AND o.OrderDate >= '2023-04-01'
    ) AS CumulativeSales
    ORDER BY orderdate;
...
SELECT
    OrderDate,
    round(SUM(Quantity * Price), 2)  AS DailySales,
    round(LAG(SUM(Quantity * Price)) OVER (ORDER BY OrderDate), 2)  AS PreviousDaySales,
    round(SUM(Quantity * Price) - LAG(SUM(Quantity * Price)) OVER (ORDER BY OrderDate), 2)  AS SalesDifference
FROM Orders
...
SELECT DISTINCT
e.EmployeeID,
round(AVG(TotalAmount) OVER (PARTITION BY e.EmployeeID), 2)  AS AvgOrderValue
FROM (
    SELECT 
        o.EmployeeID,
        o.OrderID,
